...
public class Alert {
    ...
    private static class SendableAlerts implements Sendable {
        ...
        public String[] getStrings(AlertType alertType) {
          Predicate<Alert> activeFilter = (Alert alert) -> alert.alertType == alertType && alert.active;
          Comparator<Alert> timeSorter =  (Alert a1, Alert a2) -> (int) (a2.activeStartTime - a1.activeStartTime);

          return alerts.stream()
              .filter(activeFilter)
              .sorted(timeSorter)
              .map((Alert alert) -> alert.text)
              .toArray(String[]::new);
        }
     ...
     */
    public Alert(String text, AlertType alertType) {
        this("Alerts", text, alertType);
    }
     ...
     * @param type Alert level specifying urgency.
     */
    public Alert(String group, String text, AlertType alertType) {
