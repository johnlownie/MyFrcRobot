...
public class AllianceFlipUtil {
    ...
    public static Translation2d apply(Translation2d translation) {
        if (shouldFlip()) {
            return new Translation2d(FieldConstants.LENGTH_METERS - translation.getX(), translation.getY());
        } else {
            return translation;
        }
    }
    ...
    public static double apply(double xCoordinate) {
        if (shouldFlip()) {
            return FieldConstants.LENGTH_METERS - xCoordinate;
        } else {
            return xCoordinate;
        }
    }
    ...
    public static Pose2d apply(Pose2d pose) {
        if (shouldFlip()) {
            return new Pose2d(
                    FieldConstants.LENGTH_METERS - pose.getX(), pose.getY(),
