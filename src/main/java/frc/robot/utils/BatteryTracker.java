...
public class BatteryTracker {
    private static final List<RobotType> supportedRobots = List.of(RobotType.ROBOT_2023_MK4I);
    public static final String defaultName = "BAT-0000-000";

    private static final int nameLength = 12;
    private static final byte[] scanCommand = new byte[] { 0x7e, 0x00, 0x08, 0x01, 0x00, 0x02, 0x01, (byte) 0xab, (byte) 0xcd };
    ...
    public static String scanBattery(double timeout) {
        if (RobotConstants.getMode() == Mode.REAL) {
            if (supportedRobots.contains(RobotConstants.getRobot())) {
