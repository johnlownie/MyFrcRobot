...
public class PPHolonomicDriveController {
    ...
    public ChassisSpeeds calculate(Pose2d currentPose, PathPlannerState referenceState) {
        ...
        Logger.getInstance().recordOutput("PathPlanner/xFF", xFF);
        Logger.getInstance().recordOutput("PathPlanner/yFF", yFF);
        Logger.getInstance().recordOutput("PathPlanner/rotationFF", rotationFF);
        ...
        Logger.getInstance().recordOutput("PathPlanner/translationError.getX()", this.translationError.getX());
        Logger.getInstance().recordOutput("PathPlanner/translationError.getY()", this.translationError.getY());
        Logger.getInstance().recordOutput("PathPlanner/translationError.getAngle()", this.translationError.getAngle().getDegrees());
        Logger.getInstance().recordOutput("PathPlanner/rotationError.getDegrees()", this.rotationError.getDegrees());
    
        if (!this.isEnabled) {
            return ChassisSpeeds.fromFieldRelativeSpeeds(xFF, yFF, rotationFF, currentPose.getRotation());
        }

        Logger.getInstance().recordOutput("PathPlanner/currentPose.getX()", currentPose.getX());
        Logger.getInstance().recordOutput("PathPlanner/currentPose.getY()", currentPose.getY());
        Logger.getInstance().recordOutput("PathPlanner/referenceState.poseMeters.getX()", referenceState.poseMeters.getX());
        Logger.getInstance().recordOutput("PathPlanner/referenceState.poseMeters.getY()", referenceState.poseMeters.getY());
        ...
        Logger.getInstance().recordOutput("PathPlanner/xFeedback", xFeedback);
        Logger.getInstance().recordOutput("PathPlanner/yFeedback", yFeedback);
        Logger.getInstance().recordOutput("PathPlanner/rotationFeedback", rotationFeedback);
