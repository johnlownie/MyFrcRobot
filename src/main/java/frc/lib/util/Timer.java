...
// My implementation because they didn't create an isRunning method
 ...
 *
 * <p>
 * Note that if the user calls SimHooks.restartTiming(), they should also reset
 * the timer so
...
public class Timer {
     ...
     * a specified period
     * of time given in seconds. Motors will continue to run at their last assigned
     * values, and
     ...
     * from the current
     * system clock the start time stored in the timer class. If the clock is not
     * running, then return
     ...
     * all time requests
     * should be relative to the system clock. Note that this method is a no-op if
     * the timer is
     * already running.
     */
    public void start() {
     ...
     * resetting the
     * accumulated time, then starting the timer again. If you want an event to
     * periodically reoccur
     * at some time interval from the start time, consider using advanceIfElapsed()
     * instead.
     */
    public void restart() {
        if (m_running) {
     ...
     * time by that period.
     * This is useful to decide if it's time to do periodic work without drifting
     * later by the time it
